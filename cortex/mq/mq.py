...
class RabbitDriver():
	...
	def subscribe_to_exchange(self, exchange_name, echange_type, q_name='')
		self.create_exchange(exchange_name, echange_type)
		result = self.create_queue(q_name)
...
class MQer:
	...
	def subscribe_to_exchange(self, exchange_name, queue_name=''):
		self.driver.subscribe_to_exchange(exchange_name, exchange_type, queue_name)
	...
	def connect_to_consume_function(self, queue_name, callback_function, auto_ack = True):
		self.driver.connect_to_function(self,qeueue_name, callback_function, auto_ack)
