...
class MongoDriver():
	...
	def first_one(self, user_id, datetime):
		result = self.users.find_one({'_id':user_id, 'datetime':datetime}) #this implies someone has submitted something to this user_id + datetime combo already
		x = self.users.find()
		for z in x:
	...
	def save(self, data_name ,data):
		...
		user_id = int(data['user']['userId'])
		datetime = data['datetime']
		if self.first_one(user_id, datetime):
			self.users.insert_one({'_id':user_id, 'datetime':datetime})
			print(term.red_on_white(f'{data_name} is here first, metada has been set'))
		
		data_dic = data[data_name]
		search_dic = {'_id':user_id, 'datetime':datetime}
